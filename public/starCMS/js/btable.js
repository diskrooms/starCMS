...
layui.define(['element', 'common', 'paging', 'form'], function (exports) {
    ...
    function getTpl(options) {
        ...
        if (options.checkbox) {
            tds = '<td><input type="checkbox" data-item="id" data-id="{{ item.' + options.field + ' }}" lay-skin="primary" /></td>' + tds;
        } else {
            tds = '<td style="display:none;"><input type="hidden" data-item="id" value="{{ item.' + options.field + ' }}" data-id="{{ item.' + options.field + ' }}" name="id" /></td>' + tds;
        }
        tpl += '<tr>' + tds + '</tr>'
        tpl += '{{# }); }}';
        tpl += '{{# }else{ }}';
